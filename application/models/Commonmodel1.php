...
{ 
   ...
   function getViewData($field)
   {
	   $this->db->select('category.category,post_news.id,post_news.Title,post_news.Contents,post_news.picture,post_news.Category,post_news.Type_Post,post_news.Priority,post_news.user_id,post_news.publishers,post_news.status');
       $this->db->from('post_news');	  
	   $this->db->order_by("post_news.id", "desc");	
       $this->db->join('category', 'post_news.category = category.id');
   ...
   {
	   $a=0; 	   
       $multipleWhere = array('category.category' => $id, 'post_news.subCategory!=' => $a);
