...
class Admin extends CI_Controller
{
    ...
    { 
		  ...
		  if($sessarr['status']!=0)
		  {
		    ...
		    $result['sessarrs1']    = $sessarr;
		    $result['menu']         =  $this->Commonmodel->getAllMenu('menu',$sessarr['id']);
		    $data['reportercount']  =  $this->Commonmodel->reporter_counts();
		    $this->load->view('header',$result); 
    ...
    { 
	   ...
	   {	
          ...
          $sessarr              =  $this->session->userdata('sessdata');
          $post                 =  array('recent' => 0); 			
          $data['results']      =  $this->Commonmodel->UpdateData('users',$post,$id);	      
	      $data['results']      =  $this->Commonmodel->getRecord('users','reporter');
	      $result['sessarrs']   =  $sessarr['status']; 
	      $result['sessarrs1']  =  $sessarr;
	      $data['msg']          = "Record In Active Successfully";
	      $result['menu']       =  $this->Commonmodel->getAllMenu('menu',$sessarr['id']);
	      $this->load->view('header',$result);
	      $this->load->view('viewReporter',$data);
	   }
   ...
   public function ActiveReporter()
    { 
       $sessarr              =  $this->session->userdata('sessdata');	  
       if($sessarr['status']!=0)
	   {
	      ...
	      $result['sessarrs']   =  $sessarr['status']; 
	      $result['sessarrs1']  =  $sessarr;
	      $data['msg']          =  "Record Active Successfully";
	      $result['menu']       =  $this->Commonmodel->getAllMenu('menu',$sessarr['id']);
	      $this->load->view('header',$result);
	      $this->load->view('viewReporter',$data);
       }
...
//--------------------------------------------------------------------------------------------------------------------------------	
    ...
    public function ActiveSubadmin()
    {       
          $sessarr              =  $this->session->userdata('sessdata');	 
          if($sessarr['status']!= 0)
	      { 
    ...
    {  	   
	   ...
	   {
          $data['error']         = "Only jpg, jpeg, gif and png files are allowed.";         
		  $this->load->view('header',$result);
		  $this->load->view('Registration',$data);
		  $add =""; 
	   }
   ...
   public function editrepoter()
   {
    ...
    public function deletesReporterPost()
    {        		
       $sessarr               =  $this->session->userdata('sessdata'); 
	   if($sessarr['status']!=0)
	   { 
    ...
    {	
        ...
        if($sessarr['status']!=0)
	    {
		  ...
		  $sessarr              = $this->session->userdata('sessdata');
		  $data['result']       = $this->Commonmodel->getsingleData('post_news',$id);	
		  $data['results']      = $this->Commonmodel->getcategory('category',$postty);
		  $result['sessarrs']   = $sessarr['status']; 
		  $result['sessarrs1']  = $sessarr;
		  $result['menu']       = $this->Commonmodel->getAllMenu('menu',$sessarr['id']);
	      $this->load->view('header',$result);
		...
		{ 		  
		 ...
		 {		  
		   ...
		   { 
              ...
              if(!in_array($ext,$allowed))
		      {
			    ...
			    {
				  $data['error'] = "Image File should be select.";   
			    }
		 ...
		 {			 
		   ...
		   {              
			...
			//$result    = preg_replace('/[^a-zA-Z0-9- ]/s','',$Title);
            //$news_url  = strtolower(str_replace(" ","-",$result)); 
            //$news_url = "";
			//$status          =  0;
            ...
            else
            {
            	...
            	$reporter_name   = $sessarr['reporter_name'];
            	$role            =  $sessarr['type'];
            }
			...
			$post = array(
						   ...
						   'reporter_name'   =>  $reporter_name,
						   'news_url'        =>  strtolower(str_replace(" ","-", preg_replace('/[^a-zA-Z0-9- ]/s','',$Title))),
						   'news_type'       =>  $NewsType
			 ); 
    ...
    public function viewBlog()
    {
	   ...
	   {          
		 $limit='';
		 $data['results']      =  $this->Commonmodel->getViewBlog('blog',$sessarr['id'],$sessarr['status']);		 
		 $result['sessarrs']   =  $sessarr['status'];
         $result['sessarrs1']  =  $sessarr; 
         $result['menu']       =  $this->Commonmodel->getAllMenu('menu',$sessarr['id']);		  
		 $data['msg']          =  "";
		    ...
		    {			  
			  ...
			  else
			  {
			  	  ...
			  	  if("Top"==$this->input->post('img_position')) 
                  {
				     ...
				     if("300" >$width || "470" >$height)
				     { 
                      
					   $id                   =  $this->input->post('editid');
                       $result['result']     =  $this->Commonmodel->getsingleData('advertise',$id);			  
			           $result['id']         =  $id;			 
					   $result['error']  = "image size must be 300 x 470 pixels.";                     
                       $add              = "";
					   $result['msg']        =  "";
				       $result['menu']       =  $this->Commonmodel->getAllMenu('menu',$sessarr['id']);
					   $this->load->view('editadvertise',$result);
                  ...
                  else if("Center"==$this->input->post('img_position'))
				  {
				     if($pic=='')
			         {			         
                       $width  =  800;
